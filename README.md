...
_exeCustomFontSelected = "None"
local _exeFontMy = nil
local _exeFontBad = nil
local _exeFontConn = nil
local _exeFontOrig = {}
local setCustomFontVisual = nil
local EXE_FONT_NAMES = {"None","Coding Font","Summer","Beachy","Scary","Bangers"}
local _exeFontValLabel = nil
local function _exeFontTouch(this)
    if this:IsA("TextLabel") or this:IsA("TextButton") or this:IsA("TextBox") then
        ...
        return cur == _exeFontBad or string.find(cur, "BuilderIcons")
    end
    return true
end
local function _exeFontChange(txt)
    if (txt:IsA("TextLabel") or txt:IsA("TextButton") or txt:IsA("TextBox")) and not _exeFontTouch(txt) then
        if not _exeFontOrig[txt] then _exeFontOrig[txt] = txt.FontFace end
        pcall(function() txt.FontFace = _exeFontMy end)
    end
end
local function _exeFontSetup()
    if _exeFontMy then return true end
    local ok = pcall(function()
        if isfile and writefile and getcustomasset then
            if not isfile("exe_starborn.ttf") then
                writefile("exe_starborn.ttf", game:HttpGet("https://granny.anondrop.net/uploads/6c2505542959f371/Starborn.ttf"))
            end
            writefile("exe_starborn.json", HS:JSONEncode({
                name = "Starborn",
                faces = {{name="Regular",weight=400,style="normal",assetId=getcustomasset("exe_starborn.ttf")}}
            }))
            _exeFontMy = Font.new(getcustomasset("exe_starborn.json"))
            _exeFontBad = tostring(Font.new("rbxasset://LuaPackages/Packages/_Index/BuilderIcons/BuilderIcons/BuilderIcons.json"))
        end
    end)
    return ok and _exeFontMy ~= nil
end
local function getExeFontFor(name)
    if name == "None" then return nil end
    if name == "Coding Font" then
        if _exeFontSetup() then return _exeFontMy end
        return nil
...
local function applyExeFont(name)
    if _exeFontConn then _exeFontConn:Disconnect(); _exeFontConn = nil end
    for obj, origFont in pairs(_exeFontOrig) do
        pcall(function() if obj and obj.Parent then obj.FontFace = origFont end end)
    end
    _exeFontOrig = {}
    _exeCustomFontSelected = name or "None"
    if name and name ~= "None" then
        local font = getExeFontFor(name)
        if font then
            _exeFontMy = font
            for _, v in pairs(game:GetDescendants()) do _exeFontChange(v) end
            _exeFontConn = game.DescendantAdded:Connect(function(obj)
                if _exeCustomFontSelected ~= "None" then _exeFontChange(obj) end
            end)
        end
    end
end
...
-- ============================================================
-- INFINITE JUMP (Green Duels platform-based version)
-- ============================================================
...
RunService.Heartbeat:Connect(function()
	...
	if not char:FindFirstChildOfClass("Tool") then
		-- Same as original .exe: look for "Bat" specifically in backpack
		local bp=LP:FindFirstChildOfClass("Backpack") or LP:FindFirstChild("Backpack")
	...
	-- Auto Swing - activate equipped tool, try "Bat" first then any tool
...
setAutoSwitchSpeedVisual,setAutoTurnOffSpeedVisual,setAutoSwitchLaggerSpeedVisual,setRagdollTimerVisual,setFpsBoostVisual,setTryhardVisual,setZombieVisual,setFovVisual=nil,nil,nil,nil,nil,nil,nil,nil
fovValueBox=nil
...
-- âš™ï¸ PODER EXACTO: 23 - 32 - 70 - 90 (CRAZY)
local NIVELES = {
    Low     = { poder = 23 },
    Mid     = { poder = 32 },
    High    = { poder = 70 },
...
-- ðŸ”‘ TECLA PREDETERMINADA: M (SE GUARDA Y SE CARGA DE LA CONFIGURACIÃ“N)
...
-- âš ï¸ LAG ENGINE (PRESERVADO EXACTAMENTE IGUAL AL ORIGINAL)
...
local function actualizarSwitch()
    ...
    if toggleClick then
        -- MODIFICADO: Mostrar ON si estÃ¡ activo, y OFF si estÃ¡ inactivo
...
local function actualizarCandado()
    if lockButton then
        -- MODIFICADO: Mostrar Locked si estÃ¡ bloqueado, y Unlocked si estÃ¡ desbloqueado
...
-- Panel Principal (SÃ³lido, Sin contorno, Taller)
...
-- ðŸ–¼ï¸ FONDO PERSONALIZADO ROSA Y NEGRO
...
bgOverlay.BackgroundTransparency = 0.82 -- Overlay mÃ¡s transparente para mostrar mejor la imagen
...
-- InicializaciÃ³n de estados visuales
actualizarBotonesNivel()
actualizarSwitch()
actualizarCandado()
...
-- â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
-- ACTIVACIÃ“N CON LA TECLA CONFIGURADA
-- â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
UserInputService.InputBegan:Connect(function(input, gp)
    if not screenGui.Parent then return end
    if gp then return end
    -- Evita disparar el lagger si estÃ¡s escribiendo una tecla en la TextBox
...
--// NUKE OPTIMIZER (Green Duels style)
if nukeOptimizerEnabled==nil then nukeOptimizerEnabled=false end
nukeOptimizerConns=nukeOptimizerConns or {}
setNukeOptimizerVisual=setNukeOptimizerVisual or nil

function nukeSafeDestroy(obj)
	pcall(function() if obj and obj.Parent and obj.Name~="Overhead" then obj:Destroy() end end)
end
function nukeApplyObject(obj)
	pcall(function()
		...
		elseif obj:IsA("Texture") or obj:IsA("Decal") then
			if not (obj.Name=="face" and obj.Parent and obj.Parent.Name=="Head") then nukeSafeDestroy(obj) end
		elseif obj:IsA("SpecialMesh") then
			pcall(function() obj.TextureId="" end)
		elseif obj:IsA("SurfaceAppearance") then
			nukeSafeDestroy(obj)
		elseif obj:IsA("ParticleEmitter") or obj:IsA("Trail") or obj:IsA("Beam") or obj:IsA("Smoke") or obj:IsA("Fire") or obj:IsA("Sparkles") or obj:IsA("Explosion") or obj:IsA("PointLight") or obj:IsA("SpotLight") or obj:IsA("SurfaceLight") then
...
function enableNukeOptimizer()
	if nukeOptimizerEnabled then return end
	nukeOptimizerEnabled=true
	pcall(function() settings().Rendering.QualityLevel=Enum.QualityLevel.Level01 end)
	...
	pcall(function()
		...
		for _,v in ipairs(Lighting:GetChildren()) do
			if v:IsA("BloomEffect") or v:IsA("BlurEffect") or v:IsA("ColorCorrectionEffect") or v:IsA("SunRaysEffect") or v:IsA("DepthOfFieldEffect") or v:IsA("Atmosphere") or v:IsA("Clouds") then nukeSafeDestroy(v) end
		end
		local sky=Lighting:FindFirstChild("_ExeNukeSky") or Instance.new("Sky")
		sky.Name="_ExeNukeSky";sky.SkyboxBk="";sky.SkyboxDn="";sky.SkyboxFt="";sky.SkyboxLf="";sky.SkyboxRt="";sky.SkyboxUp="";sky.CelestialBodiesShown=false;sky.Parent=Lighting
	end)
	pcall(function()
		local T=workspace.Terrain
		T.Decoration=false;T.WaterWaveSize=0;T.WaterWaveSpeed=0;T.WaterReflectance=0;T.WaterTransparency=1
	end)
	for _,obj in ipairs(workspace:GetDescendants()) do if not nukeOptimizerEnabled then return end;nukeApplyObject(obj) end
	table.insert(nukeOptimizerConns,workspace.DescendantAdded:Connect(function(obj)
		if nukeOptimizerEnabled then task.defer(function() if nukeOptimizerEnabled then nukeApplyObject(obj) end end) end
	end))
	table.insert(nukeOptimizerConns,Lighting.DescendantAdded:Connect(function(obj)
		if nukeOptimizerEnabled and (obj:IsA("Atmosphere") or obj:IsA("Clouds") or obj:IsA("PostEffect")) then nukeSafeDestroy(obj) end
	end))
end
function disableNukeOptimizer()
	nukeOptimizerEnabled=false
	for _,c in ipairs(nukeOptimizerConns) do pcall(function() c:Disconnect() end) end
	nukeOptimizerConns={}
end
function setNukeOptimizer(on)
	if on then enableNukeOptimizer() else disableNukeOptimizer() end
	if setNukeOptimizerVisual then setNukeOptimizerVisual(on) end
	pcall(saveConfig)
end
...
--// .EXE MOBILE BUTTONS (no images; pink/black style)
...
function resetMobileButtonPositions()
	-- Destroy current mobile buttons without saving their old dragged positions.
	...
	-- Rebuild at default positions, then immediately write the full config.
	buildMobileButtons()
	pcall(saveConfig);saveMobileButtonsConfigOnly()
	task.delay(0.2,function() pcall(saveConfig);saveMobileButtonsConfigOnly() end)
	-- EXE post-reset mobile save confirmation
end


-- EXE mobile button dedicated autosave
task.spawn(function()
...
function buildMobileButtons()
	...
	local RED2=Color3.fromRGB(100,18,70)
	local BG=Color3.fromRGB(8,8,12)
	local OFF=RED
	local DIM=Color3.fromRGB(120,120,132)
	local W=Color3.fromRGB(245,245,245)
	local ON_BG=Color3.fromRGB(20,20,24)
	local defs={
		...
		{top="AUTO",bot="TP",key="autoTP",layout="sideBySide"},
	...
	local g=Instance.new("ScreenGui");g.Name="ExeMobileButtons";g.ResetOnSpawn=false;g.DisplayOrder=20;g.IgnoreGuiInset=true
	...
	local grpDragging,grpDragStart,grpStartPos=false,nil,nil
	container.InputBegan:Connect(function(input)
		if not mobileMovingMode then return end
		if input.UserInputType==Enum.UserInputType.MouseButton1 or input.UserInputType==Enum.UserInputType.Touch then
			grpDragging=true;grpDragStart=input.Position;grpStartPos=container.Position
			input.Changed:Connect(function() if input.UserInputState==Enum.UserInputState.End then grpDragging=false;saveMobileButtonPositions();saveMobileButtonsConfigOnly() end end)
		end
	end)
	UIS.InputChanged:Connect(function(input)
		if grpDragging and (input.UserInputType==Enum.UserInputType.MouseMovement or input.UserInputType==Enum.UserInputType.Touch) then
			local d=input.Position-grpDragStart;container.Position=UDim2.new(grpStartPos.X.Scale,grpStartPos.X.Offset+d.X,grpStartPos.Y.Scale,grpStartPos.Y.Offset+d.Y);mobBtnSavedContainerPos={xs=container.Position.X.Scale,xo=container.Position.X.Offset,ys=container.Position.Y.Scale,yo=container.Position.Y.Offset};saveMobileButtonsConfigOnly()
		end
	end)
	for i,def in ipairs(defs) do
		...
		local btnBg=Instance.new("ImageLabel",b)
		btnBg.Name="ButtonBackground";btnBg.Size=UDim2.new(1,0,1,0);btnBg.Position=UDim2.new(0,0,0,0)
		btnBg.BackgroundTransparency=1;btnBg.Image="rbxthumb://type=Asset&id=129219792103700&w=420&h=420";btnBg.ScaleType=Enum.ScaleType.Crop;btnBg.ImageTransparency=0.18;btnBg.ZIndex=1
		Instance.new("UICorner",btnBg).CornerRadius=getCorner()
		local st=Instance.new("UIStroke",b);st.Color=Color3.fromRGB(25,25,30);st.Thickness=1.4;st.Transparency=0.15
		...
		local bot=Instance.new("TextLabel",b);bot.BackgroundTransparency=1;bot.Text=def.bot;bot.TextColor3=RED;bot.Font=Enum.Font.GothamBlack;bot.TextSize=math.max(7,math.floor(9*mobileButtonsSize/100));bot.TextStrokeTransparency=0;bot.TextStrokeColor3=Color3.fromRGB(0,0,0);bot.ZIndex=3
		...
		local function setOn(state)
			...
			st.Color=state and RED or Color3.fromRGB(25,25,30)
			st.Thickness=state and 3.2 or 1.4
			st.Transparency=state and 0 or 0.15
			if btnBg then btnBg.ImageTransparency=state and 0.05 or 0.18 end
			top.TextColor3=W
			bot.TextColor3=W
		end
		...
		elseif def.key=="autoTP" then setOn(autoTPEnabled)
		...
		hit.InputBegan:Connect(function(input)
			if input.UserInputType==Enum.UserInputType.MouseButton1 or input.UserInputType==Enum.UserInputType.Touch then dragging=true;moved=false;dragStart=input.Position;startPos=mobileMovingMode and container.Position or b.Position end
		end)
		hit.InputChanged:Connect(function(input)
			if not dragging then return end
			if input.UserInputType==Enum.UserInputType.MouseMovement or input.UserInputType==Enum.UserInputType.Touch then
				local d=input.Position-dragStart;if math.abs(d.X)>5 or math.abs(d.Y)>5 then moved=true end
				if mobileMovingMode and moved then
						container.Position=UDim2.new(startPos.X.Scale,startPos.X.Offset+d.X,startPos.Y.Scale,startPos.Y.Offset+d.Y)
						mobBtnSavedContainerPos={xs=container.Position.X.Scale,xo=container.Position.X.Offset,ys=container.Position.Y.Scale,yo=container.Position.Y.Offset}
						saveMobileButtonsConfigOnly()
					elseif mobileDragSmall and moved then
		...
		hit.InputEnded:Connect(function(input)
			if not dragging then return end
			if input.UserInputType==Enum.UserInputType.MouseButton1 or input.UserInputType==Enum.UserInputType.Touch then
				if moved and (mobileDragSmall or mobileMovingMode) then saveMobileButtonPositions();saveMobileButtonsConfigOnly()
				elseif not moved and not mobileMovingMode then
					...
					elseif def.key=="autoTP" then autoTPEnabled=not autoTPEnabled;if autoTPEnabled then startAutoTP() else stopAutoTP() end;if setAutoTPVisual then setAutoTPVisual(autoTPEnabled) end;setOn(autoTPEnabled);saveConfig();saveMobileButtonsConfigOnly()
...
function saveConfig()
	...
	local cfg={
		...
		antiLag=antiLagEnabled,stretchRez=stretchRezEnabled,fpsBoostEnabled=fpsBoostEnabled,nukeOptimizer=nukeOptimizerEnabled,ragdollTimer=ragdollTimerEnabled,
		...
		fovEnabled=fovEnabled,fovValue=fovValue,skyTheme=currentSkyTheme,animePack=currentAnimePack,customFont=_exeCustomFontSelected,
...
-- EXE mobile extra autosave hooks
...
local function startAutoSwitchSpeed()
	...
	autoSwitchSpeedConn=RunService.Heartbeat:Connect(function()
		...
		-- Auto switch lagger speed: when speed drops to threshold, turn on lagger carry
...
local function buildGui()
	...
	setAutoSwitchSpeedVisual=mkToggle(movPage,"Auto Switch Speed",function(on) autoSwitchSpeedEnabled=on;if on or autoTurnOffSpeedEnabled or autoSwitchLaggerSpeedEnabled then startAutoSwitchSpeed() else stopAutoSwitchSpeed() end;saveConfig() end)
	setAutoSwitchLaggerSpeedVisual=mkToggle(movPage,"Auto Switch Lagger Speed",function(on) autoSwitchLaggerSpeedEnabled=on;if on or autoSwitchSpeedEnabled or autoTurnOffSpeedEnabled then startAutoSwitchSpeed() else stopAutoSwitchSpeed() end;saveConfig() end)
	setAutoTurnOffSpeedVisual=mkToggle(movPage,"Auto Turn Off Speed",function(on) autoTurnOffSpeedEnabled=on;if on or autoSwitchSpeedEnabled or autoSwitchLaggerSpeedEnabled then startAutoSwitchSpeed() else stopAutoSwitchSpeed() end;saveConfig() end)
	...
	mkSect(movPage,"TP Down")
	mkActionButton(movPage,"TP Down","TP",function() runTPFloor() end)
	setAutoTPVisual=mkToggle(movPage,"Auto TP Down",function(on) autoTPEnabled=on;if on then startAutoTP() else stopAutoTP() end;saveConfig() end)
	do local row=mkRow(movPage,32);mkLabel(row,"Auto TP Down Height");autoTPHeightBox=mkBox(row,autoTPHeight,55,62,function(v) if v>=0 and v<=500 then autoTPHeight=v else autoTPHeightBox.Text=tostring(autoTPHeight) end;saveConfig() end) end
	...
	setNukeOptimizerVisual=mkToggle(utilPage,"Nuke Optimizer",function(on) setNukeOptimizer(on) end)
	setFpsBoostVisual=mkToggle(utilPage,"FPS Boost",function(on) fpsBoostEnabled=on;if on then applyFPSBoost() end;saveConfig() end)
	_exeFontValLabel=mkArrowSelector(utilPage,"Custom Font",EXE_FONT_NAMES,_exeCustomFontSelected,function(name) applyExeFont(name);saveConfig() end)
	...
	mkSect(setPage,"Mobile Btns Configuration")
	do
		local row=mkRow(setPage,32);mkLabel(row,"Move Buttons")
		local moveBtn=Instance.new("TextButton",row)
		moveBtn.Size=UDim2.new(0,96,0,22);moveBtn.Position=UDim2.new(1,-102,0.5,-11)
		moveBtn.BackgroundColor3=INP;moveBtn.BorderSizePixel=0;moveBtn.Text=mobileMovingMode and "LOCK BUTTONS" or "MOVE BUTTONS";moveBtn.TextColor3=RED
	...
	mkActionButton(setPage,"Reset Buttons","Reset",function() resetMobileButtonPositions() end)
	do
		local row=mkRow(setPage,32);mkLabel(row,"Btn Scale")
		...
		local function refresh(sel)
			for _,r in ipairs(refs) do
				local a=r.sz==sel
				r.btn.BackgroundColor3=a and Color3.fromRGB(100,18,70) or INP
				r.btn.TextColor3=a and RED or W
			end
		end
		for i,sz in ipairs(sizes) do
			...
			sb.BackgroundColor3=(mobileButtonsSize==sz) and Color3.fromRGB(100,18,70) or INP;sb.BorderSizePixel=0
			sb.Text=tostring(sz);sb.TextColor3=(mobileButtonsSize==sz) and RED or W;sb.Font=Enum.Font.GothamBlack;sb.TextSize=8;sb.ZIndex=5
	...
	setMobileDragSmallVisual=mkToggle(setPage,"Drag Small Menu",function(on) mobileDragSmall=on;pcall(saveConfig);saveMobileButtonsConfigOnly() end)
	if mobileDragSmall and setMobileDragSmallVisual then setMobileDragSmallVisual(true) end
	do
		mkSect(setPage,"Btn Shape")
		local shapes={{"circle","Circle"},{"box","Box"},{"straight","Straight"}}
		local row=mkRow(setPage,42)
		for i,sh in ipairs(shapes) do
			local btn=Instance.new("TextButton",row)
			btn.Size=UDim2.new(0,80,0,28);btn.Position=UDim2.new(0,8+(i-1)*88,0.5,-14)
			btn.BackgroundColor3=(mobileButtonShape==sh[1]) and Color3.fromRGB(100,18,70) or INP
			btn.BorderSizePixel=0;btn.Text=sh[2]:upper();btn.TextColor3=(mobileButtonShape==sh[1]) and RED or W;btn.Font=Enum.Font.GothamBlack;btn.TextSize=8;btn.ZIndex=5
			...
			btn.Activated:Connect(function()
				...
				for _,sib in ipairs(row:GetChildren()) do
					if sib:IsA("TextButton") then
						...
						sib.BackgroundColor3=active and Color3.fromRGB(100,18,70) or INP
						sib.TextColor3=active and RED or W
					end
				end
...
local function loadConfigKeys()
	...
	if cfg.customFont and type(cfg.customFont)=="string" then _exeCustomFontSelected=cfg.customFont end
...
local function loadConfigState()
	...
	task.spawn(function()
		...
		if cfg.nukeOptimizer then enableNukeOptimizer();if setNukeOptimizerVisual then setNukeOptimizerVisual(true) end end
		if cfg.stretchRez then enableStretchRez();if setStretchRezVisual then setStretchRezVisual(true) end end
		if cfg.customFont and cfg.customFont ~= "None" then task.spawn(function() task.wait(0.3);applyExeFont(cfg.customFont) end) end
	end)
end
loadConfigKeys()
buildGui()
